...
function Inspector(divid, diagram, options) {
  ...
  diagram.addModelChangedListener(function(e) {
    var currentNode = e.object; //当前点击的模型元素
    if (currentNode === null) return;
    if (e.isTransactionFinished) {
      if (currentNode.category !== "ClassDiagram" && currentNode.category !== "SequenceDiagram") {
        self.inspectObject();
      }
    }
  });
  if (this.inspectsSelection) {
    var selectedNode = diagram.firstSelectedNode(); //当前点击的模型元素
    if (selectedNode.category !== "ClassDiagram" && selectedNode.category !== "SequenceDiagram") {
      self.inspectObject();
    }
    
    diagram.addDiagramListener("ObjectDoubleClicked", function(e) {
      var currentNode = e.subject.part.data; //当前点击的模型元素
      if (currentNode.category !== "ClassDiagram" && currentNode.category !== "SequenceDiagram") {
        self.inspectObject();
      }
    });
...
Inspector.prototype.inspectObject = function(object) {
  ...
  if (data.category === "ClassDiagram" || data.category === "SequenceDiagram") return;
  ...
  if (this.includesOwnProperties) {
    for (var k in data) {
      if (k !== "text" && k !== "category" && k !== "key" && k !== "description") continue;
  ...
  textarea.style.width = "96.5%";
  textarea.style.height = "100px";
  textarea.value = data.description;
  textarea.id = "infoDescription";
...
function addClassItem(elementId) {
  let classPropertiesDiv = document.getElementById(elementId);
  let newDiv = document.createElement("div");
  let visibility = document.createElement("input");
  visibility.style.width = "45px";
  let name = document.createElement("input");
  name.style.marginRight = "10px";
  newDiv.appendChild(visibility);
  newDiv.appendChild(name);
  
  let button = document.createElement("button");
  button.style.display = "none";
  button.onclick = function() {
    //获取当前按钮的父元素并将包括父元素在内的所有元素均删除
  ...
  classPropertiesDiv.appendChild(newDiv);
}
//显示/隐藏删除条目的按钮
function removeClassItem(elementId) {
  let classPropertiesDiv = document.getElementById(elementId);
  let buttonArray = classPropertiesDiv.getElementsByTagName("button");
  let display;
  if (buttonArray[0].style.display === "none") {
    display = "inline";
  } else {
    display = "none";
  }
  ...
  for (let i=0; i<buttonArray.length; i++) {
    buttonArray[i].style.display = display;
  }
}
/*classDiagram框 End */
